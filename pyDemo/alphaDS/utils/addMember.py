...
member = client.get_list_members('1644339127071162369').data
if member:
...
def add_member():
    with requests.session() as session:
        while True:
            user_id = q_add.get(block=True)
            # for user_id in users:
            print(user_id)
            if user_id not in already and client.get_user(id=user_id):
            ...
            # 添加失败重新放入队列
            if int(user_id) not in [i.id for i in client.get_list_members('1644339127071162369').data]:
                print('添加失败,重新放入队列')
                q_add.put(user_id)
                time.sleep(300)
            else:
