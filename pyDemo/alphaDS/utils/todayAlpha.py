...
def do_task():
    ...
    #         alpha.append(i)
    #         alpha2.append(i['tweet_alpha'])
    bot.send_message(-1001982993052, '\n\n'.join([time.strftime('%Y-%m-%d').replace('-','\-'),"🚀Today's Alpha"]+['['+i['tweet_user']+']'+'(https://twitter\.com/'+i['tweet_user']+')'+' @['+i['tweet_alpha']+']'+'(https://twitter\.com/'+i['tweet_alpha']+')  \|  [' + i['alpha_time'].replace('-', '\-') +']'+'(https://twitter\.com/'+ i['tweet_user']+ '/status/' + i['tweet_id']+')' for i in alpha1]),parse_mode="MarkdownV2",disable_web_page_preview=False)
# 设置每天的 8 点执行任务
# do_task()
