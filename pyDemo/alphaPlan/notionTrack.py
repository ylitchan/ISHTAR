...
messages=[{"role": "assistant", "content":"发售时间:true 1970年1月1日UTC时间下午0点,发射时间:true 1970年1月1日UTC时间下午0点"}]
attach='\n提取以上内容中的发射或者发售信息，如果正在发售或者发射或者ido则在对应项写true和正在发射或者发售，如果没有相关信息则在对应项写false，如果有具体发射或者发射时间则在对应项写true和时间'
...
async def get_tweet(user_id,username):
    ...
    tweet = res.data[0]
    tweet_id = tweet.id
    if not collection.find_one({'tweet_id': tweet_id}) and re.findall(r'launch|sale|release|live|list|发射|fire|available|open',tweet.text,re.I):
        msg = messages + [{"role": "user", "content": tweet.text + attach}]
        #print(msg)
        hf = await chatGPT(msg)
        if 'true' in hf and '1970' not in hf:
